...
def SearchWeather(request):
	...
	if JSONResponse['cod'] == '404':
		print(
			"Error! An unknown location was entered into\n"
			"the weather application search bar.")
	...
	'''
	Pass the variable "context" to the
	"SearchCityWeather.html" file, so
	that it can be accessed inside that
	template.'''
	return render(
		request, 'weather/search-weather.html', context)
# def SearchWeather(request):
